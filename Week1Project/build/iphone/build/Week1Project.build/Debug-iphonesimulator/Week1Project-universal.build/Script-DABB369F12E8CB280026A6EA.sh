...
if [ "x$TITANIUM_CLI_XCODEBUILD" == "x" ]; then
    /usr/local/bin/node "/Users/aboynton/.appcelerator/install/5.2.0/package/node_modules/titanium/lib/titanium.js" build --platform iphone --sdk "4.1.0.GA" --no-prompt --no-progress-bars --no-banner --no-colors --build-only --xcode
    exit $?
else
